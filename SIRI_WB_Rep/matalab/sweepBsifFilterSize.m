...
outDir = fullfile(projectRoot, 'data', 'processed', 'bsif_sweep_nocrop');
...
%% Sweep setting
filterSizes = [3 5 7 9 11 13 15 17];
fixedBits = 7;

%% Process each filter size
for fs = filterSizes
    fprintf('\nExtracting BSIF filter=%dx%d, bits=%d\n', fs, fs, fixedBits);

    numFeatures = 2 ^ fixedBits;
    X = zeros(n, numFeatures);

    bsifOpt.vdiv = 1;
    bsifOpt.hdiv = 1;
    bsifOpt.filter = fs;
    bsifOpt.bits = fixedBits;
    bsifOpt.mode = 'nh';
    ...
    outName = sprintf('balu_bsif_%dx%d_%dbit_nocrop.csv', fs, fs, fixedBits);
    outPath = fullfile(outDir, outName);

    save_feature_csv(meta, X, sprintf('bsif_%dx%d_%dbit_', fs, fs, fixedBits), outPath);

    fprintf('Saved: %s\n', outPath);
end

fprintf('\nFilter-size sweep finished.\n');
