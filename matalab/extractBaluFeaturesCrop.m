...
outDir = fullfile(projectRoot, 'data', 'processed', 'balu');
...
% BSIF default: 5x5, 7-bit, 128 features
bsif57Opt.vdiv = 1;
bsif57Opt.hdiv = 1;
bsif57Opt.filter = 5;
bsif57Opt.bits = 7;
...
% BSIF optimized: 7x7, 8-bit, 256 features
bsif78Opt.vdiv = 1;
bsif78Opt.hdiv = 1;
bsif78Opt.filter = 7;
bsif78Opt.bits = 8;
...
% HOG: 20 x 10 cells, 9 bins = 1800 features
hogOpt.nj = 20;
hogOpt.ni = 10;
hogOpt.B = 9;
hogOpt.show = 0;
...
for i = 1:n
    ...
    Icrop = crop_foreground_resize(I, 256, 10);

    % LBP
    [x_lbp, ~] = Bfx_lbp(Icrop, [], lbpOpt);
    X_lbp(i, :) = double(x_lbp(:))';

    % BSIF 5x5 7-bit
    [x_bsif57, ~] = Bfx_bsif(Icrop, [], bsif57Opt);
    X_bsif57(i, :) = double(x_bsif57(:))';

    % BSIF 7x7 8-bit
    [x_bsif78, ~] = Bfx_bsif(Icrop, [], bsif78Opt);
    X_bsif78(i, :) = double(x_bsif78(:))';

    % HOG
    [x_hog, ~] = Bfx_hog(Icrop, hogOpt);
    X_hog(i, :) = double(x_hog(:))';
...
save_feature_csv(meta, X_lbp, 'lbp_', ...
    fullfile(outDir, 'balu_lbp59_crop.csv'));

save_feature_csv(meta, X_bsif57, 'bsif57_', ...
    fullfile(outDir, 'balu_bsif_5x5_7bit_crop.csv'));

save_feature_csv(meta, X_bsif78, 'bsif78_', ...
    fullfile(outDir, 'balu_bsif_7x7_8bit_crop.csv'));

save_feature_csv(meta, X_hog, 'hog_', ...
    fullfile(outDir, 'balu_hog_crop.csv'));

% Combined LBP + optimized BSIF
X_lbp_bsif78 = [X_lbp, X_bsif78];

save_feature_csv(meta, X_lbp_bsif78, 'lbp_bsif78_', ...
    fullfile(outDir, 'balu_lbp59_bsif_7x7_8bit_crop.csv'));

fprintf('\nAll Balu features saved to:\n%s\n', outDir);

%% Local functions

function Iout = crop_foreground_resize(I, resizeSize, padding)
    mask = I > 0;
    [y, x] = find(mask);
    ...
    if isempty(x)
        ...
        x1 = max(min(x) - padding, 1);
        x2 = min(max(x) + padding, size(I, 2));
        y1 = max(min(y) - padding, 1);
        y2 = min(max(y) + padding, size(I, 1));

        Icrop = I(y1:y2, x1:x2);
    end

    Iout = imresize(Icrop, [resizeSize resizeSize]);
end
